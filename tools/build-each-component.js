...
const sdConfig = {
    source: [
        `${BASE_PATH}/properties/**/*.json5`,
        `${BASE_PATH}/components/**/*.json5`
    ],
    platforms: {
        css: {
            buildPath: `dist/design-tokens/web/components/styles/`,
            transformGroup: 'kbq/css',
            filter: (token) => !['light', 'dark', 'font', 'size', 'typography', 'md-typography', 'palette']
...
const dictionaryMapper = (dictionary, outputReferences) => {
    const formatProperty = formatHelpers.createPropertyFormatter(
        {outputReferences, dictionary, format: 'css'}
    );
...
StyleDictionary.registerFormat({
    ...
    formatter: function ({dictionary, options = {}}) {
        ...
        baseDictionary.allTokens = baseDictionary.allProperties = baseDictionary.allTokens
            .filter((token) => token.attributes.light
                || token.attributes.type === 'size'
                || token.attributes.font);
        darkDictionary.allTokens = darkDictionary.allProperties = darkDictionary.allTokens.filter((token) => token.attributes.dark);
...
const main = async () => {
    try {
        const files = await fs.readdir(
            path.join(process.cwd(), 'packages', 'design-tokens', 'web', 'components'),
            {encoding: 'utf8'});

        sdConfig.platforms.css.files = files.filter((file) => path.extname(file).includes(TOKEN_FILE_EXT))
            .map((currentValue) => ({
